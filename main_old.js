...
function populatePrestamosSelect(clienteFiltro = '') {
  ...
  // 🔹 Llenar préstamos filtrados por cliente
  for (const pid in prestamosByUser) {
    const p = prestamosByUser[pid];
    if (clienteFiltro && p.clienteId !== clienteFiltro) continue;
    const clientName = clientsById[p.clienteId]?.nombre || p.clienteId;

    // Calcular saldo real considerando cuotas pagadas
    const cuotasTotal = (cuotasByPrestamo[pid] || [])
      .filter(c => !c.pagada)
      .reduce((sum, c) => sum + (Number(c.monto) || 0) - (Number(c.pagado_parcial) || 0), 0);
    const abonadoTotal = abonosByPrestamo[pid] || 0;
    const saldoPendiente = cuotasTotal;

    // 🔹 No mostrar préstamos completamente saldados
    if (saldoPendiente <= 0) continue;
...
$id("abonoForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    await runTransaction(db, async (transaction) => {
      ...
      // 🔹 Sumar solo las cuotas NO pagadas (las que realmente faltan)
      const totalPendienteCuotas = cuotasDocs
        .filter(c => !c.pagada)
        .reduce((sum, c) => sum + (Number(c.monto) || 0) - (Number(c.pagado_parcial) || 0), 0);

      const totalAbonadoPrevio = abonosByPrestamo[prestamoId] || 0;

      // 🔹 El saldo pendiente real será lo que falta por pagar en cuotas no saldadas
      const saldoPendiente = totalPendienteCuotas;
...
function refreshHistorial() {
  ...
  for (const pid in prestamosByUser) {
    ...
    // ✅ Calcular saldo pendiente real según cuotas NO pagadas
    const cuotas = cuotasByPrestamo[pid] || [];
    const totalPendiente = cuotas
      .filter(c => !c.pagada)
      .reduce((sum, c) => sum + (Number(c.monto) || 0) - (Number(c.pagado_parcial) || 0), 0);

    const saldo = totalPendiente;
    const totalAb = abonosByPrestamo[pid] || 0;
      ...
      if (expRow.style.display === "none") {
        ...
        body.querySelectorAll(".btn-toggle-cuota").forEach(bt => {
          ...
          bt.onclick = async (e2) => {
            ...
            const cuotas = (cuotasByPrestamo[pid] || []).sort((a, b) => (a.numero || 0) - (b.numero || 0));
...
document.getElementById("btnGuardarRegularizacion")?.addEventListener("click", async () => {
  ...
  try {
    // 🔹 Ordenar cuotas por número antes de recalcular fechas
    const cuotas = (cuotasByPrestamo[pid] || []).slice().sort((a, b) => (a.numero || 0) - (b.numero || 0));

    // 🔹 Calcular nuevas fechas en orden mensual correcto
    const nuevasFechas = cuotas.map((c, i) => {
      const f = new Date(fechaInicio);
      f.setMonth(f.getMonth() + i);
      return f;
